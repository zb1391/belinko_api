...
describe GooglePlacesApi do
  describe "#get_isntance_type" do
    it "returns an ArgumentError when an unknown type is passed" do
      expect{GooglePlacesApi::get_instance_type("badtype")}.to raise_error(ArgumentError)
    end

    it "returns the class of the searcher" do
      expect(GooglePlacesApi::get_instance_type("text")).to eql(GooglePlacesApi::TextSearch)
    end
  end
end
